...
class RoadEnvironment(arcade.Window):
    ...
    def __init__(self, width, height, title, car1=(0, 0), car2=(1,0), riders=[], shared_robot_states=[0]):
        ...
        self.car_states = shared_robot_states
        ...
        if True or all(self.car_states):
            ...
            if self.riders:
                if car1_request == car2_request == CarRequest.RIDER:
                    ...
                    self.car2.path = nx.shortest_path(self.grid, self.car2.current_position(), self.riders.remove(rider))[rider.start]
                elif car1_request == CarRequest.RIDER:
                    rider = self.closest_rider(self.car1)
                    self.car2.path = nx.shortest_path(self.grid, self.car1.current_position(), self.riders.remove(rider))[rider.start]
        self.resync_grid_with_sprites()
    ...
    def _handle_car_conflict(self):
        ...
        if car1_closest_rider == car2_closest_rider:
            c1_length, c2_length = nx.shortest_path_length(self.grid, car1, car2_closest_rider.start), nx.shortest_path_length(self.grid, car2, car2_closest_rider.start)
